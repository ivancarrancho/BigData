...
from config import COUCHBASE_FTS_MEMORY_QUOTA_MB
from config import COUCHBASE_INDEX_MEMORY_QUOTA_MB
from config import COUCHBASE_MEMORY_QUOTA_MB
...
COUCHBASE_DEFAULT_PASSWORD = 'password'
...
def get_cluster_couchbase_url(
    ...
    return f'couchbase://{host}:{port}?fetch_mutation_tokens={fetch_mutation_tokens}&operation_timeout={operation_timeout}&n1ql_timeout={n1ql_timeout}'
...
def setup_couchbase_services(*, cluster_url, username, password):
    ...
    return (
        ...
        'cannot change node services after cluster is provisioned' in
        r.text
    )
...
def import_couchbase_default_data(*, cluster_url, username, password):
    ...
    return (
        ...
        f'Sample bucket {COUCHBASE_DEFAULT_DATASET} is already loaded.' in
        r.text
    )
...
def config_couchbase(
    *,
    ...
    allowed_username = get_allowed_username(username=username)
    assert (
        username == allowed_username
    ), 'Not a valid username: {}, a valid one would be {}'.format(
        username,
        allowed_username,
    )
